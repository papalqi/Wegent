...
class TestDockerExecutor:
    ...
    @patch("executor_manager.executors.docker.executor.find_available_port")
    @patch("executor_manager.executors.docker.executor.build_callback_url")
    @patch("executor_manager.executors.docker.executor.generate_executor_name")
    @patch("executor_manager.executors.docker.executor.get_container_ports")
    def test_submit_executor_existing_container_no_ports(
        self, mock_ports, mock_name, mock_callback, mock_port, executor
    ):
        """Test recreating executor container when existing container has no ports"""
        task = {
            "task_id": 123,
            "subtask_id": 456,
            "user": {"name": "test_user"},
        ...
        mock_port.return_value = 8080
        mock_callback.return_value = "http://callback.url"
        executor.subprocess.run.return_value = MagicMock(
            stdout="container-id\n", returncode=0
        )
        ...
        assert result["status"] == "success"
        assert result["executor_name"] == "new-executor"

    @patch("executor_manager.executors.docker.executor.find_available_port")
    @patch("executor_manager.executors.docker.executor.build_callback_url")
    @patch("executor_manager.executors.docker.executor.generate_executor_name")
    @patch("executor_manager.executors.docker.executor.get_container_ports")
    def test_submit_executor_existing_container_request_error_recreates(
        self, mock_ports, mock_name, mock_callback, mock_port, executor
    ):
        """Test recreating executor container when existing container is unreachable"""
        task = {
            "task_id": 123,
            "subtask_id": 456,
            "user": {"name": "test_user"},
        ...
        executor.requests.post.side_effect = requests.ConnectionError("boom")

        mock_name.return_value = "new-executor"
        mock_port.return_value = 8081
        mock_callback.return_value = "http://callback.url"
        executor.subprocess.run.return_value = MagicMock(
            stdout="container-id\n", returncode=0
        )
