...
class ExecutorDispatcher:
    ...
    _executors: Dict[str, Any] | None = None
    ...
    @classmethod
    def _get_executors(cls) -> Dict[str, Any]:
        if cls._executors is None:
            cls._executors = cls._load_executors()
    ...
    def get_executor(cls, task_type: str):
        ...
        logger.info(f"Available executors: {list(executors.keys())}")

        if task_type not in executors:
            logger.warning(
                f"Executor type '{task_type}' not found, using default 'docker' executor"
            )
            if "docker" not in executors:
                logger.error(
                    f"Default 'docker' executor not found in available executors: {list(executors.keys())}"
                )
                raise ValueError(f"Default 'docker' executor not found")
            return executors["docker"]

        logger.info(f"Found executor for type '{task_type}': {executors[task_type]}")
