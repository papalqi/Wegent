...
def build_callback_url(task: dict) -> str:
    ...
    callback_host_env = os.getenv("CALLBACK_HOST", "")
    callback_port_env = os.getenv("CALLBACK_PORT", "")
    callback_url_env = os.getenv("CALLBACK_URL", "")

    # Priority:
    # 1) task.callback_url (explicit per-task override)
    # 2) CALLBACK_HOST/CALLBACK_PORT (explicit host/port override)
    # 3) CALLBACK_URL (prebuilt url)
    if not callback_url and (callback_host_env or callback_port_env):
        # Get the host IP that's accessible from containers
        # 127.0.0.1 won't work for container-to-container communication
        callback_host = callback_host_env or get_host_ip()
        callback_port = callback_port_env or "8001"
    ...
    elif not callback_url and callback_url_env:
