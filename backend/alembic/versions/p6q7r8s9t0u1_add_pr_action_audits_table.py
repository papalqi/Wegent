...
def upgrade():
    ...
    if not inspector.has_table("pr_action_audits"):
        op.create_table(
            "pr_action_audits",
    ...
    inspector = sa.inspect(connection)
    index_names = {idx["name"] for idx in inspector.get_indexes("pr_action_audits")}

    if "ix_pr_action_audits_user_id" not in index_names:
        op.create_index(
    ...
    if "ix_pr_action_audits_created_at" not in index_names:
        op.create_index(
