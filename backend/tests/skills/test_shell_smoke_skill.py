...
SKILL_DIR = Path(__file__).parent.parent.parent / "init_data" / "skills" / "shell_smoke"
...
def test_shell_smoke_skill_package_layout() -> None:
    assert SKILL_DIR.exists() and SKILL_DIR.is_dir()
    assert (SKILL_DIR / "SKILL.md").exists()
    assert (SKILL_DIR / "shell_smoke.py").exists()
    assert (SKILL_DIR / "mcp_smoke_server.py").exists()


def test_shell_smoke_skill_metadata_binds_codex_and_claudecode() -> None:
